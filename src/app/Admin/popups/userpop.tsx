...
const UserPopup: React.FC<UserPopupProps> = ({ onClose }) => {
  ...
  return (
    <div className="fixed top-1/2 left-1/2 border rounded-xl shadow-lg transform -translate-x-1/2 -translate-y-1/2 bg-white w-[90%] max-w-md h-[45vh] p-2 z-50">
      ...
      {nameError && <div className="text-red-500 text-xs mx-6">{nameError}</div>}
      ...
      {emailError && <div className="text-red-500 text-xs mx-6">{emailError}</div>}
        </div>
        <div className="flex w-full h-12 justify-end">
          <button onClick={handleSave} className="flex mx-2 hover:bg-[#6528F7] hover:border-none justify-center items-center border text-black font-semibold w-20 h-10 rounded-lg hover:text-white">
            Save
          </button>
          <button
            onClick={onClose}
